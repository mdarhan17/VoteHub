...
def dashboard():
    ...
    cur.execute("SELECT * FROM elections WHERE status='active' ORDER BY start_datetime DESC")
...
def profile():
    ...
    if request.method == "POST":
        ...
        if not date_of_birth or not gender or not address or not id_type or not id_number:
        ...
        if voter:
            cur.execute("""
                UPDATE voters
                SET date_of_birth=%s, gender=%s, address=%s, id_type=%s, id_number=%s,
                    verification_status='pending'
                WHERE user_id=%s
            """, (date_of_birth, gender, address, id_type, id_number, user_id))
        else:
            ...
            """, (user_id, voter_uid, date_of_birth, gender, address, id_type, id_number))

        mysql.connection.commit()
        cur.close()

        flash("Profile submitted successfully. Waiting for admin approval.", "success")
        return redirect(url_for("voter.dashboard"))

    cur.close()
    return render_template("voter/profile.html", user=user, voter=voter)
...
def history():
    ...
    return render_template("voter/history.html", history=history)
